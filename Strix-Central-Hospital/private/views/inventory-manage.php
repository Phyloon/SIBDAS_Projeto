<?php 
// Load configuration and start session for flash messages
require_once '../../config/config.php';
session_start();

// Fetch all non-deleted equipment for inventory display
$stmt = $pdo->query("SELECT * FROM equipamentos WHERE deleted_at IS NULL");
$allEquipments = $stmt->fetchAll(PDO::FETCH_ASSOC);

// Fetch all suppliers (used for dropdowns in modals)
$stmt = $pdo->query("SELECT * FROM fornecedores ORDER BY nome_empresa");
$fornecedores = $stmt->fetchAll(PDO::FETCH_ASSOC);

// Calculate equipment status counts for the progress bar
...
// Compute percentages for progress segments
...
<!-- Display session-based success/error messages -->
...
<!-- Include header (starts HTML and top navigation) -->
...
<div class="d-flex">
    ...
    <div id="content">
        ...
        <!-- Top bar with user info, notifications, etc. -->
        <?php include '../includes/topbar.php'?>

        <!-- SECTION: Inventory Status (progress bar + legend) -->
        <div class="card border-0 shadow-sm mb-4" style="border-radius: 12px;">
            <div class="card-body p-4">
                ...
                <div class="d-flex flex-wrap align-items-center gap-3 filter-bar-modal">
                    ...
                    <!-- Filter dropdown (group, availability, department, criticality) -->
                    ...
                    <!-- Manage Inventory dropdown (Add, Modify, Delete) -->
                    <div class="dropdown">
        ...
        <!-- SECTION: Equipment Cards Grid -->
        <!-- Each card shows equipment details and opens a modal on click -->
        <div class="row mb-4">
            <?php foreach ($allEquipments as $eq): ?>
            <div class="col-3 mb-4" 
                 data-group="<?= htmlspecialchars($eq['grupo']) ?>" 
                 data-availability="<?= htmlspecialchars($eq['estado']) ?>" 
                 data-department="<?= htmlspecialchars($eq['departamento']) ?>" 
                 data-criticality="<?= htmlspecialchars($eq['criticidade']) ?>">
                <div class="card h-100" style="cursor: pointer;" data-bs-toggle="modal" data-bs-target="#modal_<?= $eq['id'] ?>">
                    ...
                    <ul class="list-group list-group-flush text-center">
                        ...
                        <li class="list-group-item">
                            <?= htmlspecialchars(("Wing: " . $eq['location_wing'] ?? '') . " || Floor: " . ($eq['location_floor'] ?? '') . " || Room: " . ($eq['location_room'] ?? '')) ?>
                        </li>
                    </ul>
                </div>
            </div>
       ...
       <!-- SECTION: Include All Modals -->
        <!-- These are separate files for cleaner code -->
        <!-- Modal for detailed equipment view (triggered by card click) -->
        <?php include '../includes/modal_card_equipamento.php'; ?>
        
        <!-- Modal for adding new equipment -->
        <?php include '../includes/new_equipment.php'?>

        <!-- Modal for modifying existing equipment -->
...
<!-- Include footer (closing HTML, scripts, etc.) -->
