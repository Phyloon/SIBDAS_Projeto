<?php 
// Load helper functions (includes redirect_if_not_logged, etc.)
require_once 'includes/funcoes.php';
session_start();
// Ensure user is logged in
redirect_if_not_logged(); 

// Only allow tech and ceo roles to view this dashboard
...
// ============================================================
// 1. Fetch all documents with equipment details (for alerts)
// ============================================================
$today   = new DateTime();
...
// Arrays to hold upcoming maintenance and warranty alerts
$upcomingMaintenance = [];
$upcomingWarranty    = [];

// Loop through each contract/document to find upcoming dates
foreach ($rawContracts as $contract) {

    // --- Calculate next maintenance date based on periodicidade ---
    if (!empty($contract['data_inicio_garantia']) && !empty($contract['periodicidade'])) {
        ...
        // Keep only if within the next 7 days
        if ($proxima <= $in7days) {
...
// ============================================================
// 2. Chart data queries (must be placed OUTSIDE the foreach loop)
// ============================================================

// A. Equipment count by location wing
$stmtLoc = $pdo->query("
...
// C. Top 5 suppliers (using many-to-many relation)
$stmtForn = $pdo->query("
...
// D. Count equipment with no associated documents
    ...
    <div id="content">
        ...
        <div class="container-fluid p-4">
            <div class="row g-4">
                ...
                <!-- Left column: Upcoming appointments (maintenance & warranty alerts) -->
                <div class="col-xl-7">
                    <div class="card h-100">
                        <div class="card-body">
                            ...
                            <div class="appointment-list">
                                ...
                                <!-- Show each warranty expiry alert -->
                ...
                <!-- Right column: Status chart and missing docs card -->
                <div class="col-xl-5 d-flex flex-column gap-3">
                    <!-- Equipment status overview (pie/doughnut chart) -->
                    ...
                    <!-- Quick stats: equipment with missing documentation -->
            ...
            <!-- Include various modals (used elsewhere in the app) -->
...
<!-- Pass chart data to JavaScript via a global object -->
<script>
    const dashboardData = {
        locLabels: <?= $locLabels ?? '[]' ?>,
        locCounts: <?= $locCounts ?? '[]' ?>,
